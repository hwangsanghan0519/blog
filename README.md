...
CELEB HOUSE은 소셜 미디어에서 화제가 된 셀럽 상품을 콘텐츠로 설명하고, 사용자가 신뢰할 수 있는 구매 경로까지 빠르게 연결하는 에디토리얼 커머스입니다.
...
2. `Editorial trust` — 표지, 본문, 셀럽하우스 네컷, 설명을 통해 구매 전 맥락을 제공합니다.
...
- 상품 상세 다이얼로그, 이미지 갤러리, 셀럽하우스 네컷, 본문, 판매처 링크
...
stateDiagram-v2
  [*] --> draft: 새 상품
  draft --> published: 셀럽하우스 네컷 4장+설명 충족
...
`published`만 공개 화면과 SEO 응답에 포함됩니다. 발행 시 셀럽하우스 네컷 이미지와 설명 네 쌍을 모두 요구해 불완전한 상세 페이지가 노출되는 것을 막습니다.
...
새로고침 직후 React가 실행되기 전에는 `index.html`의 CELEB HOUSE 로고·레드 스피닝 로더가 표시됩니다. React가 마운트된 뒤 캐시가 없는 첫 방문에서는 starter 상품이나 샘플 광고를 렌더하지 않고, 원격 스냅샷이 확정될 때까지 실제 화면과 유사한 높이의 스켈레톤을 사용합니다. 검증 가능한 상품·설정 캐시가 모두 있는 재방문은 캐시를 즉시 보여주고 백그라운드에서 최신 데이터를 동기화합니다.

```mermaid
flowchart TD
  A[HTML 응답] --> A1[CELEB HOUSE 로고 + 스피닝 로더]
  A1 --> A2[React mount]
  A2 --> B{유효한 상품·설정 캐시?}
  B -->|있음| C[캐시 화면 즉시 렌더]
  B -->|없음| D[스토어프런트 스켈레톤]
...
부트 로더와 스켈레톤은 상태 label을 제공하고 `prefers-reduced-motion`에서는 회전·shimmer 애니메이션을 끕니다. JavaScript가 비활성화된 경우에는 `<noscript>`의 서비스 설명이 대신 표시됩니다.
...
| 셀럽하우스 네컷 | `detailImages[4]`, `detailDescriptions[4]` | 고정 네 컷 상품 스토리 |
...
- 프로젝트 이름과 명령을 `celeb-house-commerce` 기준으로 정리했습니다.
...
```mermaid
timeline
  title CELEB HOUSE Commerce engineering roadmap
  현재 : 도메인 명칭 정리
       : 상태·API·정규화 계층 분리
       : 핵심 단위 테스트와 품질 게이트
  다음 : 스토어프런트 섹션 컴포넌트화
